...
function getNames(presidentsArr) {
  return presidentsArr.map((e) => e.name);
}
...
function getDemocraticPresidents(presidentsArr) {
  return presidentsArr.filter((e) => e.party === "Democratic");
}
...
function getAffiliatedPresidents(presidentsArr) {
  return presidentsArr.filter((e) => e.party);
}
...
function countYearsInOffice(presidentsArr) {
  return presidentsArr.reduce((a, b) => {
    if (b.leftOffice) {
      return a + (b.leftOffice - b.tookOffice);
    } else {
      return a;
    }
  }, 0);
...
function getAgeAtInauguration(presidentsArr) {
  return presidentsArr.map((e) => {
    let ageAtInauguration = e.tookOffice - e.birthYear;
    e.ageAtInauguration = ageAtInauguration;
    return e;
  });
}
